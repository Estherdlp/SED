...
void config_GPIO(){						//Configuracion puertos micro
	...
	GPIOA -> MODER &= ~(1 << (6*2 +1));	//Configuracion GPIOA-6 como salida digital
	GPIOA -> MODER |= (1 << (6*2));		//Salida LED verde

	GPIOA -> MODER &= ~(1 << (7*2 +1));	//Configuracion GPIOA-7 como salida digital
	GPIOA -> MODER |= (1 << (7*2));		//Salida LED rojo

	GPIOA -> OTYPER &= ~(1 << 5);		//Configuracion salida push pull GPIOA-5
	GPIOA -> OTYPER &= ~(1 << 6);		//Configuracion salida push pull GPIOA-6
	GPIOA -> OTYPER &= ~(1 << 7);		//Configuracion salida push pull GPIOA-7

	GPIOA -> PUPDR &= ~(1 << (5*2+1));	//Configuracion salida flotante GPIOA-5
	GPIOA -> PUPDR &= ~(1 << (5*2));
	GPIOA -> PUPDR &= ~(1 << (6*2+1));	//Configuracion salida flotante GPIOA-6
	GPIOA -> PUPDR &= ~(1 << (6*2));
	GPIOA -> PUPDR &= ~(1 << (7*2+1));	//Configuracion salida flotante GPIOA-7
	GPIOA -> PUPDR &= ~(1 << (7*2));
}
		...
		if (alarm == 1){							//Lectura botones de seleccion D1D0 al pulsar save
			...
			if (!(GPIOA -> IDR & GPIO_PIN_1)){
				if (!(GPIOA -> IDR & GPIO_PIN_0)){	//D1D0: 0b00
					options = 1;
					GPIOA -> ODR = (0<<7 | 0<<6);	//Apagar ambos led
				}
				else{								//D1D0: 0b01
					options = 2;
					GPIOA -> ODR = (0<<7 | 1<<6);	//Encender led verde
				}
			}
			else{									//D1D0: 0b10
				if (!(GPIOA -> IDR & GPIO_PIN_0)){
					options = 3;
					GPIOA -> ODR = (1<<7 | 0<<6);	//Encender led rojo
				}
				else{								//D1D0: 0b11
					options = 4;
					GPIOA -> ODR = (1<<7 | 1<<6);	//Encender ambos led
				}
			}
