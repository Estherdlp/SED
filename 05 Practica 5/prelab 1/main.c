...
#include "main.h"
#include "GPIO_CONF.h"
#include "GPIO_InitExti.h"
#include "InitNVIC.h"
#include "ConfigADC.h"
#include "ConfigTimer3.h"

#include "stdio.h"							//Librerias para imprimir por pantalla
#include "string.h"

int valor_ADC[4] = {0, 0, 0, 0};			//Variable para volcar el valor del ADC
float media = 0;							//Var para calculo media mediciones
int pulsador = 0;							//Control pulsador on/off
int finConversion = 0;						//Control fin 4 conversiones
int indice = 0;								//Posicion vector resultados
float tension = 0.0;						//Calculo tension en el puerto
char msg_res[100];
...
void EXTI15_10_IRQHandler() {				//Interrupcion boton usuario
	...
	if (EXTI -> PR & (1 << 13)) { 			//Comprobar si ha saltado la EXTI por PC13
		if (pulsador == 0){
			...
			TIM3 -> CR1 &= ~(1 << 0);		//Parar timer 3 (no realizar mas conversiones)
			pulsador = 0;
		}
	}
...
int main(void)
{
  ...
  sprintf(msg_res, "\r\nBienvenido al prelaboratorio 1");
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  if (pulsador == 1 && finConversion == 1){
		  ...
		  media = (valor_ADC[0] + valor_ADC[1] + valor_ADC[2] + valor_ADC[3]) / 4.0; //Obtencion de la media de los raw del ADC
		  tension = media * 3.3 / 4095;												 //Calculo del valor de la tension

		  sprintf(msg_res, "\r\nValor de tension medido: %f", tension);				 //Imprimir por pantalla el resultado
		  HAL_UART_Transmit(&huart2, (char *) msg_res, strlen(msg_res), 100);

		  finConversion = 0;														 //Variable auxiliar a 0 para realizar otras 4 mediciones
	  }
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
