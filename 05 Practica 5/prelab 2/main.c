...
int start = 0;								//Control on/off uart
...
char msg_res[100];							//Tamaño mensaje
...
int main(void)
{
  ...
  sprintf(msg_res, "\r\nBienvenido al prelaboratorio 2\r\nPulse 1 para iniciar la conversion y 2 para finalizar");
  ...
  {
    /* USER CODE END WHILE */
	  HAL_UART_Receive_IT(&huart2, (uint8_t *)datoUART, 1);
	  if (start == 1 && finConversion == 1){
		  media = (valor_ADC[0] + valor_ADC[1] + valor_ADC[2] + valor_ADC[3]) / 4.0;
