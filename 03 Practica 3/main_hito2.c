...
void ConfigTimer2(){						//Parametros temporizador 2 (PWM motor)
	...
	TIM2 -> DIER = 1;						//No generar interrupcion al llegar al fin de la cuenta
}
...
int main(void)
{
  ...
  /* USER CODE END 2 */
  sprintf(msg_res, "\r\nEjercicio con sensor HC SR-04 y servomotor: ");
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  if (pulsador == 1){					//Si se ha pulsado el boton de usuario, iniciar barrido con mediciones
		  ...
		  if (estado != 0){							//Imprimir los resultados de las medidas
			  if (distancia >= 400){				//Mostrar error si el valor es mas de 4 metros
				  sprintf(msg_res, "\r\nError en la medicion.");
			  }
			  else {
				  sprintf(msg_res, "\r\nLa distancia en cm de la medida %d es: %f",estado, distancia);
			  }
			  HAL_UART_Transmit(&huart2, (char *) msg_res, strlen(msg_res), 100);
		  }

		  if (pmin > 2450){					//Si ha realizado un barrido completo, situar el sensor en el centro
			  TIM2 -> CCR2 = 1450;
			  estado = 0;
			  pulsador = 0;
