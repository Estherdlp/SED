...
void ConfigADC(){							//Parametros ADC
	...
	ADC1 -> CR2 |= (1 << 1); 				//Bit CONT = 1 para conversion continua
...
void ConfigTimer4(){						//Parametros temporizador 4 (activar conversion cada 0.25s)
	...
	TIM4 -> ARR = 2500 - 1;					//Tiempo para mostrar medida: 250 ms
...
void EXTI15_10_IRQHandler() {				//Interrupcion boton usuario
	...
	if (EXTI -> PR & (1 << 13)) { 			//Comprobar si ha saltado la EXTI por PC13
		if (pulsador == 0){
			...
			TIM4 -> CR1 |= (1 << 0);		//Arrancar timer 2 (mostrar conversion)
			pulsador = 1;
		}
		else {
			ADC1 -> CR2 &= ~(1 << 30);		//Desactivar ADC
			TIM4 -> CR1 &= ~(1 << 0);		//Parar timer 2 (no mostrar mas conversiones)
			pulsador = 0;
		}
	}
