...
void ConfigADC(){							//Parametros ADC
	...
	ADC1 -> CR1 = 0; 						//Deshabilitada interrupcion por desbordamiento, resolucion de 12 bits, un solo canal, deshabilitacion de interrupcion por fin de conversion
...
int main(void)
{
  ...
  InitNVIC();
  ConfigADC();
  sprintf(msg_res, "\r\nPulse boton de usuario para realizar conversion");
  ...
  {
    /* USER CODE END WHILE */
	  while ((ADC1 -> SR & (1 << 1)) == 0);			//Esperar hasta fin de conversion
