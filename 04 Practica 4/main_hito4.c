...
void GPIO_CONF(void){
	...
	GPIOA -> MODER |= (11 << 1*2);			//Configuracion PA1 como analogico
}
...
void ConfigADC(){							//Parametros ADC
	...
	ADC1 -> SQR3 |= (00000 << 0 | 00001 << 5);	//Canales PA0 y PA1
...
void ConfigTimer4(){						//Parametros temporizador 4 (activar conversion cada 0.5s)
	...
	TIM4 -> ARR = 10000 - 1;				//Tiempo para realizar una conversion: 1s
...
void EXTI15_10_IRQHandler() {				//Interrupcion boton usuario
	...
	if (EXTI -> PR & (1 << 13)) { 			//Comprobar si ha saltado la EXTI por PC13
		if (pulsador == 0){
			TIM4 -> CR1 |= (1 << 0);		//Arrancar timer 2 (realizar conversion)
			pulsador = 1;
		}
		else {
			TIM4 -> CR1 &= ~(1 << 0);		//Parar timer 2 (no realizar mas conversiones)
			pulsador = 0;
		}
	}
...
void ADC_IRQHandler(){						//Interrupcion ADC
	...
	if ((ADC1 -> SR & (1 << 1)) != 0) {		//Comprobar si se ha producido fin conversion
		if (posicion == 0){
			valor_ADC[posicion] = (ADC1 -> DR);	//Volcar en una variable de mas de 12 bits la conversion
			posicion = 1;
		}
		else {
			valor_ADC[posicion] = (ADC1 -> DR);	//Volcar en una variable de mas de 12 bits la conversion
...
int main(void)
{
  ...
  ConfigADC();
  ConfigTimer4();
  sprintf(msg_res, "\r\nPulse boton de usuario para iniciar conversion doble");
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  if (pulsador == 1 && finConversion == 1){
		  tension = valor_ADC[0] * 3.3 / 4095;				//Calculo del valor de la tension PA0
		  sprintf(msg_res, "\r\nValor de tension medido en el canal 0: %f", tension);
		  HAL_UART_Transmit(&huart2, (char *) msg_res, strlen(msg_res), 100);

		  tension = valor_ADC[1] * 3.3 / 4095;				//Calculo del valor de la tension PA1
		  sprintf(msg_res, "\r\nValor de tension medido en el canal 1: %f", tension);
		  HAL_UART_Transmit(&huart2, (char *) msg_res, strlen(msg_res), 100);
