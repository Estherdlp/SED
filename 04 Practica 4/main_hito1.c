...
void GPIO_CONF(void){
	...
	GPIOA -> MODER |= (10 << (1*2));		//Configuracion funcion alternativa TIM2_CH2 (led)
...
void ConfigTimer4(){						//Parametros temporizador 4 (activar conversion cada 0.5s)
	...
	TIM4 -> ARR = 1000 - 1;					//Tiempo para realizar una conversion: 1 ms
...
int main(void)
{
  ...
  ConfigTimer2();
  ConfigTimer4();
  sprintf(msg_res, "\r\nPulse boton de usuario para cambiar el brillo");
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  if (pulsador == 1 && finConversion == 1){
		  Ancho_PWM = (uint16_t)((1000/4500.0) * valor_ADC);
		  sprintf(msg_res, "\r\nObserva el cambio del brillo del LED");
		  HAL_UART_Transmit(&huart2, (char *) msg_res, strlen(msg_res), 100);
		  TIM2 -> CCR2 = Ancho_PWM - 1;					//DC para cambiar la intensidad del brillo
